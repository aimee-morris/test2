...
with st.sidebar:
    ...
    august_slump = st.checkbox("Apply 20% August Slump?", value=False)

    push_weeks_input = st.text_input("Marketing Push Weeks (comma-separated week numbers)", value="").strip()
    push_weeks = []
    if push_weeks_input:
        try:
            push_weeks = [int(x.strip()) - 1 for x in push_weeks_input.split(",") if x.strip().isdigit()]
        except:
            push_weeks = []
...
weekly_dates = [start_date + pd.Timedelta(weeks=i) for i in range(int(weeks_remaining))]
weekly_boosts = []
week_types = []

for i, date in enumerate(weekly_dates):
    boost = weekly_budget_boost
    label = ""
    if august_slump and date.month == 8:
        boost *= 0.8
        label = "August Slump"
    if i in push_weeks:
        boost *= 1.2
        label = "Marketing Push" if label == "" else label + " + Push"
    weekly_boosts.append(round(boost))
    week_types.append(label if label != "" else "Normal")
...
for date in weekly_dates:
    if august_slump and date.month == 8:
        weekly_boosts.append(round(weekly_budget_boost * 0.8))
    else:
        weekly_boosts.append(round(weekly_budget_boost))

weekly_data = pd.DataFrame({
    "Week Type": week_types,
    ...
    "Sales from Budget": weekly_boosts,
    "Projected Total Sales": np.cumsum(weekly_boosts) + current_sales
})

# Display metrics
st.markdown("### 📊 Forecast Summary")
col1, col2, col3, col4 = st.columns(4)
col1.metric("🎯 Weekly Ticket Target", f"{round(weekly_sales_target)}")
col2.metric("📈 Weekly Boost from Budget", f"{round(weekly_budget_boost)}")
